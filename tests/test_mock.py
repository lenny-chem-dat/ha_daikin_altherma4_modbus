...
def test_mock_client():
    ...
    async def run_test():
        ...
        print(
            f"Value: {result.registers[37] if len(result.registers) > 37 else 'Index out of range'}"
        )

        print("\nTesting input_52 (address 52):")
        result = await client.read_input_registers(52, 1)
        print(
            f"Value: {result.registers[52] if len(result.registers) > 52 else 'Index out of range'}"
        )

        print("\nTesting input_65 (address 65):")
        result = await client.read_input_registers(65, 1)
        print(
            f"Value: {result.registers[65] if len(result.registers) > 65 else 'Index out of range'}"
        )

        print("\nTesting holding_80 (address 80):")
        result = await client.read_holding_registers(80, 1)
        print(
            f"Value: {result.registers[80] if len(result.registers) > 80 else 'Index out of range'}"
        )
