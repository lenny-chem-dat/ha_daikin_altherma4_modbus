...
class TestLastTriggeredSensors:
    ...
    def test_all_last_triggered_sensors_definition(self):
        ...
        for expected in expected_sensors:
            assert expected in found_sensors, f"{expected} sensor not found in CALCULATED_SENSORS"
        
        # Check each sensor has correct properties
        for register_name, sensor_config in found_sensors.items():
            assert sensor_config['type'] == 'last_triggered', f"{register_name} should have type 'last_triggered'"
            assert sensor_config['device_class'] == 'timestamp', f"{register_name} should have device_class 'timestamp'"
            assert 'trigger_register_name' in sensor_config, f"{register_name} should have trigger_register_name"
            assert 'translation_key' in sensor_config, f"{register_name} should have translation_key"
            
            print(f"✓ {register_name} sensor definition found: {sensor_config['trigger_register_name']}")
    ...
    def test_trigger_registers_mapping(self):
        ...
        for sensor_name, expected_trigger in expected_mappings.items():
            ...
            assert sensor_config['trigger_register_name'] == expected_trigger, f"{sensor_name} should trigger on {expected_trigger}"
    ...
    def test_last_triggered_mechanism_simulation(self):
        ...
        for sensor_name, trigger_register, description in test_cases:
            ...
            assert is_on == True, f"{description} current value should be on"
            assert was_on == False, f"{description} previous value should be off"
    ...
    def test_last_triggered_no_trigger_scenarios(self):
        ...
        for trigger_register, description in zip(trigger_registers, descriptions):
            ...
            assert should_trigger == False, f"{description} should not trigger when staying off"
            ...
            should_trigger = is_on and not was_on
            assert should_trigger == False, f"{description} should not trigger when staying on"
            ...
            should_trigger = is_on and not was_on
            assert should_trigger == False, f"{description} should not trigger when turning off"
    ...
    def test_all_state_changes(self):
        ...
        sensors = ['last_compressor_run', 'last_defrost', 'last_booster_heater', 'last_dhw_running']
        trigger_registers = ['discrete_11', 'discrete_17', 'discrete_8', 'discrete_19']
        ...
        for sensor_name, trigger_register in zip(sensors, trigger_registers):
            for previous_val, current_val, should_trigger, action in test_scenarios:
                ...
                assert actual_trigger == should_trigger, f"Trigger mismatch for {sensor_name} when {action}"
                
                if should_trigger:
                    print(f"✓ {sensor_name}: {action} ({previous_val} -> {current_val}), trigger = {actual_trigger}")
    ...
    def test_multiple_trigger_cycles(self):
        ...
        for sensor_name, trigger_register in zip(sensors, trigger_registers):
            ...
            assert last_trigger_time == create_test_trigger_time(), f"{sensor_name} last trigger should be test time"
    ...
    def test_concurrent_triggers(self):
        ...
        expected_triggers = ['last_compressor_run', 'last_booster_heater']
        trigger_time = create_test_trigger_time()
        ...
        assert len(actual_triggers) == 2, f"Should have 2 triggers, got {len(actual_triggers)}"
    ...
    def test_integration_workflow_all_sensors(self):
        ...
        for sensor_name, trigger_register in zip(sensors, trigger_registers):
            ...
            assert should_trigger == True, f"{sensor_name} should trigger when starting"
