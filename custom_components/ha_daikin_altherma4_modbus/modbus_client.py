...
class OneBasedModbusResponse:
    ...
    def registers(self):
        ...
        if hasattr(self.original_response, 'registers'):
            ...
            result = [32766] * (max_possible_address + 1)  # +1 for 1-based indexing (dummy at index 0)
            ...
            _LOGGER.debug(f"OneBasedModbusResponse: start_address={self.start_address}, len={len(self.original_response.registers)}, result_len={len(result)}")
            return result
    ...
    def bits(self):
        ...
        if hasattr(self.original_response, 'bits'):
            # Create array large enough for all possible addresses (1-1000 as safe upper bound)
            max_possible_address = 1000  # Safe upper bound for all possible addresses
            result = [False] * (max_possible_address + 1)  # +1 for 1-based indexing (dummy at index 0)
    ...
    def is_error(self):
        ...
        return self.original_response.isError() if hasattr(self.original_response, 'isError') else False
...
class RealModbusTcpClient(ModbusClientInterface):
    ...
    async def connect(self) -> None:
        ...
        if self._reconnect_needed or not self._client.connected:
            ...
            _LOGGER.debug(f"Already connected to {self.host}:{self.port}, skipping connect")
    ...
    async def read_input_registers(self, address: int, count: int):
        ...
        async with self._lock:
            ...
            _LOGGER.warning(f"Reading input registers from address {address} with count {count}")
            original_response = await self._client.read_input_registers(address-1, count=count)
    ...
    async def read_holding_registers(self, address: int, count: int):
        ...
        async with self._lock:
            await self._ensure_connection()
            _LOGGER.warning(f"Reading holding registers from address {address} with count {count}")
            original_response = await self._client.read_holding_registers(address-1, count=count)
            return OneBasedModbusResponse(original_response, address, is_bits=False)
    ...
    async def read_discrete_inputs(self, address: int, count: int):
        ...
        async with self._lock:
            await self._ensure_connection()
            _LOGGER.warning(f"Reading discrete inputs from address {address} with count {count}")
            original_response = await self._client.read_discrete_inputs(address-1, count=count)
    ...
    async def _ensure_connection(self) -> None:
        ...
        if not self._client.connected or self._reconnect_needed:
            _LOGGER.info(f"Connection lost, attempting lazy reconnect to {self.host}:{self.port}")
            await self.connect()
