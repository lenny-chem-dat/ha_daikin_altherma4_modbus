...
class ModbusDataManager:
    ...
    def _update_coordinator_data(self, register_name: str, value: Any) -> None:
        ...
        if self.coordinator:
            if register_name in self.coordinator.data:
                ...
                self.coordinator.data.get(register_name, {})["last_updated"] = (
                    time.time()
                )
                ...
                if hasattr(self.coordinator, "async_set_updated_data"):
                    try:
                        ...
                        _LOGGER.warning(
                            f"Failed to notify coordinator of data change: {e}"
                        )
            else:
                _LOGGER.warning(
                    f"Register {register_name} not found in coordinator data"
                )
    ...
    async def _ensure_connection_and_prepare_data(self) -> dict:
        ...
        try:
            ...
            _LOGGER.error(
                f"Failed to establish Modbus connection to {self.host}:{self.port}: {e}"
            )
            if self.demo_mode:
    ...
    async def refresh_all_data(self) -> dict:
        ...
        if self.client is not None:
            ...
            _LOGGER.debug(
                f"Register refresh: Input + Discrete + Coils + Holding = {time.time() - start_time:.3f}s"
            )
        else:
            _LOGGER.warning(
    ...
    async def _fetch_input_registers(self) -> dict:
        ...
        try:
            ...
            if not self._is_modbus_error(ir1):
                data.update(
                    self._process_input_register_block(ir1, INPUT_REGISTERS, 21, 53, 21)
                )
            else:
        ...
        try:
            ...
            if not self._is_modbus_error(ir2):
                data.update(
                    self._process_input_register_block(ir2, INPUT_REGISTERS, 54, 87, 54)
                )
            else:
        ...
        for item in register_list:
            ...
            if min_address <= address <= max_address:  # In this block
                try:
                    ...
                    _LOGGER.error(
                        f"IndexError accessing register {address}: {e}, array_len={len(register_data.registers)}"
                    )
                    raise
    ...
    def _process_input_register_block(
        ...
        for (
            register_name,
            processed_item,
        ) in processed_data.items():  # Changed from unique_id to register_name
            ...
            if item.get("enum_map"):
                ...
                data[register_name] = (
                    update_value_if_changed(  # Changed from unique_id to register_name
                        register_name,
                        raw_value,
                        self.previous_data,  # Changed from unique_id to register_name
                        description,
                        input_type=input_type,
                        address=address,
                    )
    ...
    def _process_bit_sensors(
        self, result, data: dict, sensor_list, sensor_type: str
    ) -> None:
        ...
        for item in sensor_list:
            ...
            if address < len(result.bits):
                ...
                _LOGGER.warning(
                    f"{sensor_type.title()} {address} nicht im gelesenen Bereich ({len(result.bits)} Bits)"
                )

    def _process_discrete_input_sensors(self, di_result, data: dict) -> None:
        """Process discrete input sensors from Modbus result."""
        self._process_bit_sensors(
            di_result, data, DISCRETE_INPUT_SENSORS, "discrete_input"
        )
        ...
        try:
            ...
            if not self._is_modbus_error(di):
                ...
                if "exception_code=2" in error_msg:
                    _LOGGER.warning(
                        "Device does not support discrete inputs (Illegal data address)"
                    )
                    _LOGGER.info(
                        "This Daikin device does not implement discrete input registers"
                    )
                elif "exception_code=1" in error_msg:
                    _LOGGER.warning(
                        "Device does not support discrete inputs (Illegal function)"
                    )
                    _LOGGER.info(
                        "This Daikin device does not implement the discrete input function code"
                    )
                else:
                    _LOGGER.warning(
                        f"Device does not support discrete inputs or read failed: {di}"
                    )
                _LOGGER.info(
                    "Skipping discrete inputs - device may not support this register type"
                )
        except Exception as e:
            ...
            try:
                ...
                if self.client:
                    ...
                    if not self._is_modbus_error(di_retry):
                        ...
                        _LOGGER.info(
                            "Successfully retried discrete inputs after reconnection"
                        )
                    else:
                        _LOGGER.warning(
                            f"Discrete inputs retry also failed - device may not support this register type: {di_retry}"
                        )
    ...
    async def _fetch_coils(self) -> dict:
        ...
        try:
            ...
            if not self._is_modbus_error(cr):
                ...
                if "exception_code=2" in error_msg:
                    _LOGGER.warning(
                        "Device does not support coils (Illegal data address)"
                    )
                    ...
                    _LOGGER.info(
                        "This Daikin device does not implement the coil function code"
                    )
                else:
                    _LOGGER.warning(
                        f"Device does not support coils or read failed: {cr}"
                    )
                _LOGGER.info(
                    "Skipping coils - device may not support this register type"
                )
        except Exception as e:
            ...
            try:
                ...
                if self.client:
                    ...
                    if not self._is_modbus_error(cr_retry):
                        ...
                        _LOGGER.warning(
                            f"Coils retry also failed - device may not support this register type: {cr_retry}"
                        )
            except Exception as retry_e:
    ...
    async def write_holding_register(self, register_name: str, value: int) -> Any:
        ...
        try:
            ...
            if self._is_modbus_error(result):
                _LOGGER.error(
                    f"Failed to write register {register_name} (address {address}) with value {value}: {result}"
                )
                return None
            else:
                _LOGGER.debug(
                    f"Successfully wrote register {register_name} (address {address}) with value {value}"
                )
            ...
            _LOGGER.error(
                f"Exception writing register {register_name} (address {address}) with value {value}: {e}"
            )
            return None
    ...
    async def write_coil_register(self, register_name: str, value: bool) -> Any:
        ...
        try:
            ...
            if self._is_modbus_error(result):
                _LOGGER.error(
                    f"Failed to write coil {register_name} (address {address}) with value {value}: {result}"
                )
                return None
            else:
                _LOGGER.debug(
                    f"Successfully wrote coil {register_name} (address {address}) with value {value}"
                )
                # Update coordinator data directly instead of refreshing
                self._update_coordinator_data(register_name, value)
                return result
            ...
            _LOGGER.error(
                f"Exception writing coil {register_name} (address {address}) with value {value}: {e}"
            )
            return None
    ...
    async def _refresh_single_holding_register(
        ...
        try:
            ...
            raw_value = (
                result.registers[address] if len(result.registers) > address else 0
            )
            data = {"value": raw_value, "input_type": "holding", "address": address}
    ...
    async def _refresh_single_coil(self, register_id: str, address: int) -> dict:
        ...
        try:
            ...
            data = {"value": raw_value, "input_type": "coil", "address": address}
    ...
    async def _read_holding_register_block(
        ...
        try:
            ...
            block_time = time.time() - block_start
            _LOGGER.debug(
                f"Holding Register {block_name} ({count} registers) read in {block_time:.3f}s"
            )
            ...
            if not self._is_modbus_error(result):
                ...
                all_holding_registers = (
                    HOLDING_REGISTERS + SELECT_REGISTERS + HOLDING_SWITCHES
                )
                data.update(
                    self._process_holding_register_block(
                        result, all_holding_registers, min_address, max_address, offset
                    )
                )
            else:
                if is_optional:
                    _LOGGER.warning(
                        f"Device does not support holding register {block_name} (addresses {min_address}-{max_address}): {result}"
                    )
                    _LOGGER.info(
                        f"Skipping holding register {block_name} - device may not support these address ranges"
                    )
                else:
                    _LOGGER.error(
                        f"Holding Register {block_name} read failed: {result}"
                    )
            ...
            try:
                _LOGGER.info(
                    f"Attempting to re-establish connection and retry holding register {block_name}"
                )
                self.client = await ensure_modbus_connection(
                    None, self.host, self.port, self.demo_mode
                )
                if self.client:
                    retry_result = await self.client.read_holding_registers(
                        start_address, count
                    )
                    if not self._is_modbus_error(retry_result):
                        ...
                        _LOGGER.info(
                            f"Successfully retried holding register {block_name} after reconnection"
                        )
                    else:
                        if is_optional:
                            _LOGGER.warning(
                                f"Holding Register {block_name} retry also failed - device may not support these addresses: {retry_result}"
                            )
                        else:
                            _LOGGER.warning(
                                f"Holding Register {block_name} retry also failed: {retry_result}"
                            )
            except Exception as retry_e:
                _LOGGER.warning(
                    f"Retry attempt for Holding Register {block_name} failed: {retry_e}"
                )

        return data

    @staticmethod
    def _apply_register_processing(
    ...
    def _process_holding_register_block(
        self, register_data, register_list, min_address, max_address, offset
    ) -> dict:
